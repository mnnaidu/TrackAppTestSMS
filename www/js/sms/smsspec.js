var smsReader = (function (smsReader) {
	...
	var smsSpecsBySenderCode = _.groupBy(getSmsSpecs(), function (smsSpec) {
		return smsSpec.bank
	});
	var expSender = /([A-Z]){2}-[A-Z]{6}/;
	var allowedSendersByCode = _.indexBy(getAllowedSenderCodes(), function (sender) {
		return sender.code
	});
	smsReader.getSenderToTrack = function (tranSms) {
		var sender = tranSms.sender;
		return (sender && sender.length == 9 && expSender.test(sender) && (allowedSendersByCode[sender.substr(3)]));
	};
